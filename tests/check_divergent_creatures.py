...
LOOT (the `Dropable` flag) is part of that too, and is gated exactly as hard as
everything else here: a placement that disagrees with its blueprint about
whether an item drops gives loot that works exactly once per reboot -- as
placed until that creature first dies, as the blueprint says forever after.
There is NO grandfather list and no allowlist, by explicit decision: every
case fails the build until the placement and the blueprint agree. The worklist
is CLAUDE-loot-divergence-audit.md, generated by bin/audit-loot-divergence.py.
...
def main():
    ...
    # base -> list of (area, idx, loot map). Kept out of identity_key so a loot
    # mismatch reports the offending item rather than "these two tuples differ".
    ...
    # blueprint has no readable baseline. EVERY divergence fails: there is no
    # allowlist here by explicit decision.
    loot_bad = []
    ...
    if loot_bad:
        print(f"[divergent-creatures] FAIL: {len(loot_bad)} loot divergence(s) — a "
        ...
        for entry in loot_bad:
            print(f"  - {entry}", file=sys.stderr)
        print("\nFix: make the placement and the blueprint agree — decide which "
              "side is right per item (CLAUDE-loot-divergence-audit.md is the "
              "worklist, `bin/audit-loot-divergence.py` regenerates it). There "
              "is deliberately no allowlist: a divergence that is left in place "
              "is loot that behaves differently before and after the first "
              "respawn.", file=sys.stderr)
        return 1
    ...
    print(f"[divergent-creatures] OK: all {total} static creature placements respawn faithfully "
          f"({len(placements)} blueprints), loot included.")
    return 0
