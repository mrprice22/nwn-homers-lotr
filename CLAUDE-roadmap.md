...
| `docs` | no | **Where a player reads about this change**: a `docs.manual/`-relative page and anchor, e.g. `Customizations/Spells.html#soul-fatigue` or `QuestGuide.html#gloison` (`Customizations.html#<anchor>` also works — the hub forwards it), or `none` when there is nothing to explain — which is the right answer for content (new areas, items, quests, bosses): only a change to how the game works compared to stock NWN gets a Customizations entry (CLAUDE.md "What counts as a customization"). Rendered as a "How it works →" link on the public card. **Set it when you ship an Enhancement** — `bin/roadmap-lint.py` warns about any Enhancement shipped since 2026-09-23 without it, and `--docs-backlog` lists the older ones. A malformed value is only a save-time warning (never a lockout); `tests/check_customizations_hub.py` fails the repack if it names a page or anchor that does not exist. |
