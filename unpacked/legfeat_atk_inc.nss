...
// Reaping's attack and damage bonuses go through the module-wide ledger, or
// they suppress (and are suppressed by) every other bonus of the same type -
// Legendary Prowess's permanent +5 made the first two kill stacks do nothing.
...
// Reaping keeps no tag/amt/gen of its own any more: the ledger owns the running
// total, the tagged effect and the generation counter (bonus_pool_inc, source
// key BPOOL_SRC_REAPING). Sundering still has its own set below, because an AC
// DECREASE on the target is not a bonus and has nothing to pool with.
...
void LegFeatAtk_OnAttack(struct NWNX_Damage_AttackEventData data)
{
    ...
    {
        // The running total is read back out of the ledger, which is now the
        // only place it is kept. BPool_Set handles the rest: it refreshes the
        // 12 seconds, bumps the generation so an earlier kill's expiry cannot
        // cut this one short, and rebuilds the one pooled effect from the sum
        // of every live source.
        //
        // The damage half also stops being dice by accident: the old call
        // passed nAmt straight to EffectDamageIncrease, which takes a
        // DAMAGE_BONUS_* CONSTANT, so stacks 4 and 5 (+8, +10) were landing as
        // 1d8 and 2d6. The ledger converts once, on the total.
        int nAmt = BPool_Get(oAttacker, BPOOL_CH_ATTACK, BPOOL_SRC_REAPING)
                 + LEGFEAT_REAP_STEP;
        if (nAmt > LEGFEAT_REAP_MAX) nAmt = LEGFEAT_REAP_MAX;

        BPool_Set(oAttacker, BPOOL_CH_ATTACK, BPOOL_SRC_REAPING, nAmt,
                  LEGFEAT_REAP_DUR);
        BPool_Set(oAttacker, BPOOL_CH_DAMAGE, BPOOL_SRC_REAPING, nAmt,
                  LEGFEAT_REAP_DUR);
    }
