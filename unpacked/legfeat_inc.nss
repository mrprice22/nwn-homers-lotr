...
#include "nwnx_damage"
// LEGFEAT_ATK_VAR lives in legfeat_atk_inc, next to the hook that reads it:
// devcrit_atk.nss includes that file and must NOT drag the database and the
// NUI picker in behind it just to learn the name of a local variable.
...
// HOOK-kind feats are inert tokens read by combat hooks, and those hooks have
// to be armed. Two things do the arming, both from LegFeat_ArmHooks:
//
//   LEGFEAT_ATK_VAR   - a local int on the character, read by devcrit_atk.nss
//                       (the server-wide NWNX Damage ATTACK script) with one
//                       GetLocalInt per attack. That one read is the entire
//                       cost the attacker-side feats impose on every other
//                       attack on the server, and it has to stay that way.
//   legfeat_dmg       - the per-character NWNX Damage DAMAGE script, set only
//                       on characters holding a defender-side feat. Everyone
//                       else has no damage script at all and pays nothing.
const string LEGFEAT_DMG_SCRIPT = "legfeat_dmg";
...
int    LegFeat_AdoptHeldFeats(object oPC);
...
void   LegFeat_ArmHooks(object oPC);
...
// Is this item a weapon? Answered from baseitems.2da rather than a list of
// base-item constants, so CEP's and the module's own weapons count without
// anyone having to remember to add them. WeaponType is blank on everything
// that is not a weapon - shields and torches included, which is the case that
// matters for Legendary Grip.
int LegFeat_IsWeapon(object oItem)
{
    if (!GetIsObjectValid(oItem)) return FALSE;
    string sType = Get2DAString("baseitems", "WeaponType",
                                GetBaseItemType(oItem));
    return sType != "" && StringToInt(sType) > 0;
}

// A weapon in each hand. A shield or a torch in the off hand is not a weapon
// and does not qualify - Legendary Grip is paid for holding two edges, not for
// having something in the left slot.
int LegFeat_IsDualWielding(object oPC)
{
    return LegFeat_IsWeapon(GetItemInSlot(INVENTORY_SLOT_RIGHTHAND, oPC))
        && LegFeat_IsWeapon(GetItemInSlot(INVENTORY_SLOT_LEFTHAND, oPC));
}

// A bow or a crossbow, specifically - not a sling and not a thrown weapon.
// Legendary Marksman is the archer's counterpart to Legendary Onslaught, and
// GetWeaponRanged alone would hand its extra attack to a sling or a dart.
int LegFeat_IsBowArmed(object oPC)
{
    object oRight = GetItemInSlot(INVENTORY_SLOT_RIGHTHAND, oPC);
    if (!GetIsObjectValid(oRight)) return FALSE;
    int nBase = GetBaseItemType(oRight);
    return nBase == BASE_ITEM_LONGBOW || nBase == BASE_ITEM_SHORTBOW
        || nBase == BASE_ITEM_LIGHTCROSSBOW || nBase == BASE_ITEM_HEAVYCROSSBOW;
}
...
void LegFeat_ApplyOne(object oPC, int nFeatId)
{
    ...
    switch (nFeatId)
    {
        ...
        // Four immunities, applied as four effects: EffectImmunity takes one
        // IMMUNITY_TYPE_* at a time. The fifth thing Juggernaut promises -
        // that the weapon cannot be struck from the hand - is not an immunity
        // type at all and lives in legfeat_disarm.nss instead.
        case FEAT_LEGENDARY_JUGGERNAUT:
            LegFeat_ApplyEffect(oPC, EffectImmunity(IMMUNITY_TYPE_KNOCKDOWN));
            LegFeat_ApplyEffect(oPC, EffectImmunity(IMMUNITY_TYPE_ENTANGLE));
            LegFeat_ApplyEffect(oPC, EffectImmunity(IMMUNITY_TYPE_PARALYSIS));
            LegFeat_ApplyEffect(oPC, EffectImmunity(IMMUNITY_TYPE_SLOW));
            break;
        ...
        // CONDITIONAL: bow or crossbow. The mirror of Onslaught, and the pair
        // are mutually exclusive in practice - one hand, one weapon.
        case FEAT_LEGENDARY_MARKSMAN:
            if (LegFeat_IsBowArmed(oPC))
                LegFeat_ApplyEffect(oPC, EffectModifyAttacks(1));
            break;
    }
}

// Arm or disarm the combat hooks for the HOOK-kind feats this character holds.
//
// HOOK feats apply no effect, so LegFeat_ApplyAll's loop skips them entirely -
// which means this is the ONLY place their machinery gets switched on. Called
// at the end of LegFeat_ApplyAll, so it re-runs at login, on every weapon swap
// and after a re-pick, and it must therefore be safe to run repeatedly and
// must CLEAR as readily as it sets: a character who respecs out of Legendary
// Bulwark keeps its damage script otherwise, and would keep the reduction.
void LegFeat_ArmHooks(object oPC)
{
    // Attacker side: read once per attack by devcrit_atk.nss.
    int bAtk = LegFeat_HasPick(oPC, FEAT_LEGENDARY_REAPING)
            || LegFeat_HasPick(oPC, FEAT_LEGENDARY_WRATH)
            || LegFeat_HasPick(oPC, FEAT_LEGENDARY_QUARRY)
            || LegFeat_HasPick(oPC, FEAT_LEGENDARY_SUNDERING);
    if (bAtk) SetLocalInt(oPC, LEGFEAT_ATK_VAR, TRUE);
    else      DeleteLocalInt(oPC, LEGFEAT_ATK_VAR);

    // Defender side: the per-character damage event script. Setting it to ""
    // is how NWNX unregisters, so the else branch is not optional.
    int bDmg = LegFeat_HasPick(oPC, FEAT_LEGENDARY_BULWARK)
            || LegFeat_HasPick(oPC, FEAT_LEGENDARY_RIPOSTE);
    if (bDmg) NWNX_Damage_SetDamageEventScript(LEGFEAT_DMG_SCRIPT, oPC);
    else      NWNX_Damage_SetDamageEventScript("", oPC);
}
...
void LegFeat_ApplyAll(object oPC)
{
    ...
    // HOOK feats were skipped above by design; this is where they get their
    // machinery. It clears as well as sets, so a respec out of a hook feat
    // takes the hook with it.
    LegFeat_ArmHooks(oPC);
}
...
// Record any legendary feat the character already HOLDS but that this realm has
// no pick record for, and return how many were adopted.
//
// Why this exists: the feat lives in the .bic, the pick record lives in a
// PER-REALM campaign DB, and bin/sync-vault-from-prod copies .bic files from
// the live season into the dev realm without the campaign DBs. A character
// arriving that way holds feats legfeatdb has never heard of, so
// LegFeat_EnsureAllotment took its "first grant at 60" branch and offered a
// full allotment on top of what the character already had - a 4-class
// character with its one feat was handed a second (reported by Rajmund, dev
// realm, 2026-09-06). Worse, an unrecorded feat is invisible to EVERY removal
// path: LegFeat_RevokeAll iterates pick records, so respec, the class-change
// revoke and the level-drop revoke all silently skip it, stranding a RAW
// feat's +6 base score with nothing recording that it was granted.
//
// RECORD ONLY - apply nothing. A RAW feat's bonus is already in the .bic base
// scores (applied once, never at login), and EFFECT/HOOK feats are rebuilt
// from the pick records by LegFeat_ApplyAll/LegFeat_ArmHooks at the next
// login, which is exactly what adopting them enables. Applying here would add
// a second +6 on top of the one already in the character file.
int LegFeat_AdoptHeldFeats(object oPC)
{
    int nAdopted = 0;
    int i;
...
int LegFeat_EnsureAllotment(object oPC)
{
    ...
    // BEFORE any decision below, so every branch sees the character's real
    // holdings rather than only what this realm's DB happens to remember.
    // The below-60 branch then revokes an imported feat instead of ignoring
    // it, the first-grant branch subtracts it from the allotment instead of
    // granting on top of it, and the class-change branch can actually take
    // it back. Placement is the whole correctness argument here.
    LegFeat_AdoptHeldFeats(oPC);
