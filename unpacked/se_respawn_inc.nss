...
// Set on a creature that must NEVER respawn, however it dies and whatever death
// script its blueprint carries. Script-summoned creatures (x2_s2_mumdust's NPC
// Mummy Reapers) set it at creation: they are temporary by construction, and a
// blueprint whose ScriptDeath chains into nw_c2_default7 would otherwise plant a
// permanent respawning creature at the summon point (roadmap gandalfs-mummys).
const string SE_NO_RESPAWN = "SE_NO_RESPAWN";
...
void SE_DoCreatureRespawn()
{
  ...
  return;
  // Conjured, not placed: a summon is temporary by construction and must not
  // enter the world respawn pool. The flag covers script-spawned creatures with
  // no master; GetMaster covers summons, familiars, companions and henchmen.
  if (GetLocalInt(OBJECT_SELF, SE_NO_RESPAWN)) return;
  if (GetIsObjectValid(GetMaster(OBJECT_SELF))) return;
