...
void main()
{
    ...
    // iAttackResult 10 means THE ENGINE RESOLVED A DEVASTATING CRITICAL OF ITS
    // OWN, which is meant to be impossible: hak_2da/baseitems.2da has a blank
    // EpicWeaponDevastatingCriticalFeat column and the unarmed/creature feats
    // are stripped (DevCrit_ArmNoDevCrit). It is therefore an ALARM, not a
    // branch - DevCrit_AlarmEngineCrit writes one line to the server log with
    // everything needed to tell which of the two routes leaked, and tells any
    // admin online. See roadmap devcrit-unarmed-save-or-die.
    if (nResult == 10)
    {
        DevCrit_FlagNoKill(data.oTarget, oAttacker);
        DevCrit_AlarmEngineCrit(oAttacker, data.oTarget, oWeapon,
                                data.iWeaponAttackType);
        // Self-heal: whichever route leaked, an attacker still holding one of
        // the two feats the engine resolves on its own loses it here, so a leak
        // costs at most one devastating critical instead of every one for the
        // rest of the session. Free on every other attack - this branch is
        // meant never to run at all.
        DevCrit_ArmNoDevCrit(oAttacker);
    }
