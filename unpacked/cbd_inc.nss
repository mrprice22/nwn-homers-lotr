...
void   CBD_Immune(object oDummy, int nImmunityType);
...
// One permanent immunity, applied the way cbd_spawn applies all of them.
// Supernatural so nothing can dispel the dummy's protection out from under a
// trial; CBD_SelfDestruct strips the lot when the set is over.
void CBD_Immune(object oDummy, int nImmunityType)
{
    ApplyEffectToObject(DURATION_TYPE_PERMANENT,
                        SupernaturalEffect(EffectImmunity(nImmunityType)),
                        oDummy);
}
...
//
// This is the ONLY death a dummy is ever supposed to have, so cbd_spawn's
// immunities have to come off for it to land - the death immunity above all,
// but the whole set goes, because stripping by TYPE avoids caching a dozen
// effect handles across a 60-second session. Nothing else on the dummy is an
// EFFECT_TYPE_IMMUNITY, and the dummy is destroyed a moment later anyway; what
// keeps it safe in that 1.5-second window is the save bonuses on the blueprint,
// which no strip can touch.
//
