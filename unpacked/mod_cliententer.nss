...
// NOTHING IN HERE MAY CALL A *_InitDb(). Campaign-DB schema setup lives in
// onmoduleload.nss, which runs once before any player can connect. This hook
// used to re-run seven of them per login - 28 synchronous SQLite commits across
// seven DB files on the login frame, on a box with one spinning disk and a
// single-threaded server main loop - which players felt as a stall the moment
// anyone connected. Adding a table to a subsystem needs a change in
// onmoduleload.nss, not here.
void main()
{
    ...
    // Force this character's persistent UUID (stored in the .bic) to be
    // generated: it is the per-character identity key for the bestiary kill
    // records and for the play-time row below. The bestiary schema itself is
    // set up at module load (onmoduleload.nss), not here.
    GetObjectUUID(oPC);
    ...
    // Premium 2x gold/XP boosts (merit redemptions 201-204): report any
    // active/pending server + personal subscriptions - who bought each and the
    // estimated end date. Delayed past the login flood.
    ...
    // ...and give back what the strip destroyed before the proxy row existed:
    // a character that spent a feat pick on Devastating Critical (Unarmed) and
    // has nothing to show for it gets the proxy, once. Self-detecting from the
    // character's own per-level feat list, behind a one-feat guard, so it costs
    // nothing for the players it does not apply to. Roadmap: buged-dev-crit.
    // Ahead of LegFeat_ApplyAll for the same reason as the line above.
    DelayCommand(6.45, DevCrit_RestoreUnarmed(oPC));

    DelayCommand(6.5, LegFeat_ApplyAll(oPC));
    // The pooled attack/damage effect is a RENDER of the bonus ledger, never the
    // record, so it can always be redrawn. LegFeat_ApplyAll has just re-registered
    // the permanent sources; this redraws from whatever the ledger actually holds.
    // Belt and braces at login (nothing is stale there - effects and locals both
    // die with the object), and the same call is what would recover the effect
    // after anything else wipes effects out from under us.
    DelayCommand(6.6, BPool_Resync(oPC));
