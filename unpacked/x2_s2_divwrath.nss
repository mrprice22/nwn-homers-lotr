...
//:: The Divine Champion's attack bonus (+3, rising by +2 every five class
//:: levels) is registered with the per-creature bonus ledger instead of being
//:: linked as an effect - as a plain EffectAttackIncrease the whole to-hit half
//:: of the ability was worth nothing to a character carrying Legendary
//:: Prowess's permanent +5. The saves, the damage bonus and the damage
//:: reduction stay in the supernatural link, which is what the ledger
//:: witnesses.
//:: WITNESS SPELL ID IS 622, not the 621 this script's SignalEvent passes -
//:: row 621 in spells.2da is PADDING and 622 is DC_Divine_Wrath. The effects
//:: carry the id of the ability actually cast, so 622 is what
//:: GetHasSpellEffect answers to; the stock SignalEvent constant is a BioWare
//:: off-by-one and is left exactly as found.
...
void main()
{
    ...
    effect eDamage, eSaving, eReduction;
    ...
    // nAttackB is pooled, not linked - see the header note.
    ...
    effect eLink = EffectLinkEffects(eSaving, eDamage);
    ...
    BPool_SpellAttack(oTarget, BPOOL_SRC_DIVWRA, nAttackB, RoundsToSeconds(nDuration));
