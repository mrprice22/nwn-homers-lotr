// Forge gate for the "Damage Bonus" menu option: melee weapons, ranged weapons
// and gloves.
//
// Same base-item list as isweapon.nss plus BASE_ITEM_GLOVES. Gloves are the
// unarmed fighter's weapon, so a damage bonus on them is the equivalent enchant
// -- it only applies while no weapon is held, so it cannot stack with a forged
// weapon. Kept separate from isweapon.nss because that one also gates Attack
// Bonus and Massive Criticals, which stay weapons-only.
