...
### Escrow model (how spending works)

1. **Request** (`Merit_RequestById`): server-side affordability check
   (`available >= cost`, so `merit_spent` can never exceed earned), then
   `Merit_Spend` debits the cost and a `pending` row is inserted. The cost is now
   *held*.
2. **Cancel** (`Merit_CancelRedemption`): refunds via `Merit_Refund` (clamped at
   0) and sets `status='cancelled'`. A player may cancel only their own request;
   a DM may cancel any.
3. **Fulfill** (`Merit_FulfillRedemption`): sets `status='fulfilled'`; the debit
   stays. No automated reward delivery in the current proof-of-concept — a DM
   fulfils the request by hand.
...
| `VANITY` (2)    | 301–303 | vanity & swag |
| `HOUSESIZE` (3) | 401–408 | player home, by area size |
| `HOUSEFEAT` (4) | 501–505 | player home add-ons |

Every option currently ships as a **placeholder**: its label is shown with a red
`[PLACEHOLDER]` prefix (`MERIT_PH`, a literal that carries the raw colour bytes
`<c FF 01 01 >` — null-free red; see "colour bytes" below) and selecting it only
...
### To GRADUATE a placeholder to an automated reward

Deliver the effect inside `Merit_RequestById` (after the escrow `Merit_Spend`):
for an instant reward, grant it and either skip the `pending` insert or insert it
already `fulfilled`; drop the red `MERIT_PH` prefix for that option's label. Keep
the escrow accounting intact. `needs_dm` is stored per reward for exactly this
split (instant vs DM-fulfilled).
...
set of 9 conditionals. The DM list also stashes `merit_lslot_<i>_desc`.
...
- **Player:** NPC `merit_keeper` (tag/resref unchanged) — now **Barliman
  ...
  → stats → category sub-menus → option slots (`merit_cat_<c>`, `merit_osel_<i>`),
  a "my pending requests" self-cancel branch (`merit_mypend`, `merit_mcanc_<i>`),
  and a "how do I earn merit?" branch with wiki + Discord pointers (valid for
  zero-merit players). When changing the appearance/name, edit **both** the
