...
def render_catalog_nss(entries, excluded):
    ...
    body.append("int JB_CatByName(int nOrder);")
    body.append("")
    body.append("int JB_CatCount()")
    body.append("{")
    ...
    body.append("}")
    body.append("")
    body.append("// Catalogue indices in alphabetical order by name, so the picker can list")
    body.append("// tracks as a player expects to read them. The catalogue itself stays in")
    body.append("// 2DA row order, which is the order rows were allocated and must not change.")
    body.append("//")
    body.append("// Sorted here rather than in game because NWScript has no array to sort and")
    body.append("// the order never varies at run time -- it is a property of the table.")
    body.append("int JB_CatByName(int nOrder)")
    body.append("{")
    body.append("    switch (nOrder)")
    body.append("    {")
    order = sorted(range(len(entries)), key=lambda i: entries[i]["name"].lower())
    for pos, idx in enumerate(order):
        body.append("        case %d: return %d;   // %s" % (pos, idx, entries[idx]["name"]))
    body.append("    }")
    body.append("    return -1;")
    body.append("}")
    body.append("")
