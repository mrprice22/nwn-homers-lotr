...
STYLE = """  <style>
    ...
    .rm-notes pre { white-space: pre-wrap; overflow-x: auto; margin: 0.3em 0;
      font-family: ui-monospace, monospace; font-size: 0.92em; }
    /* A table inside a note (a loot table, a boss comparison). Wrapped in its
       own scroller for the same reason <pre> is: a wide table would otherwise
       stretch its card and drag the whole grid with it. */
    .rm-notes table { border-collapse: collapse; margin: 0.5em 0; font-size: 0.95em;
      display: block; max-width: 100%; overflow-x: auto; }
    .rm-notes th, .rm-notes td { border: 1px solid var(--border);
      padding: 0.3em 0.55em; text-align: left; vertical-align: top; }
    .rm-notes thead th { color: var(--fg); background: var(--card);
      font-size: 0.9em; text-transform: uppercase; letter-spacing: 0.03em; }
    .rm-notes small { font-size: 0.92em; }
