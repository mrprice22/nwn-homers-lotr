...
# bin/spellfeat_proxies.py, loaded by path like the TLK generator is.
PROXY_LIBRARY = REPO / "bin" / "spellfeat_proxies.py"
# Generated: the script-side {proxy -> real} map the runtime resolver walks.
PROXY_IDS_INC = REPO / "unpacked" / "castfeat_ids.nss"
...
# ---------------------------------------------------------------------------
# Caster feat proxies (roadmap ll-bonus-feat-lists)
#
# A second family of owned rows, appended after the legendary feats. They are
# NOT legendary feats and follow the opposite rule about selectability: a
# legendary row is unreachable from the level-up page by design, a proxy row
# exists precisely to BE reachable from it. bin/spellfeat_proxies.py carries the
# defect analysis and the emission rule; this file only renders what it derives,
# because the row indices are feat ids and only this script knows where the
# legendary block ends.
# ---------------------------------------------------------------------------

def load_proxies():
    """Import bin/spellfeat_proxies.py by path."""
    spec = importlib.util.spec_from_file_location(
        "spellfeat_proxies", PROXY_LIBRARY,
        loader=importlib.machinery.SourceFileLoader(
            "spellfeat_proxies", str(PROXY_LIBRARY)),
    )
    module = importlib.util.module_from_spec(spec)
    # Registered before exec so dataclasses inside it can resolve their own
    # module during class creation.
    sys.modules["spellfeat_proxies"] = module
...
def build_proxy_cells(proxy, proxies_by_stock, stock_cells, header, mod, gated):
    ...
    Everything that expresses a REQUIREMENT is copied verbatim - PREREQFEAT1/2,
    OrReqFeat*, REQSKILL/ReqSkillMinRanks, PreReqEpic, the ability minimums and
    ALLCLASSESCANUSE. That is what keeps the proxy honest: Automatic Still
    Spell I still demands Still Spell and 27 ranks of Spellcraft.

    Changed:
      MINSPELLLVL   -> blank. The client compares it against a maximum castable
                       spell level it resolves as 0 past class level 40, which
                       is the entire defect.
      MinLevel /    -> the band selector, 41 in this class. Not a restatement of
      MinLevelClass    the requirement; see spellfeat_proxies.py.
      LABEL /       -> unique per (feat, class).
      Constant
      SUCCESSOR     -> three cases. If the next feat in the chain has a proxy
                       for THIS class, point at that proxy so the level-up page
                       walks proxies to proxies. If it is not gated at all it
                       stays reachable through its own stock row, so keep the
                       stock value - Spell Focus succeeds to Greater Spell
                       Focus, which has no MINSPELLLVL and needs no proxy.
                       Blank only when the successor IS gated and this class
                       cannot cast high enough for it, which is the case that
                       should genuinely dead-end.

    FEAT (the name strref) and DESCRIPTION are left pointing at the stock row's
    own strings, which is why this feature adds nothing to lotr.tlk.
    """
    cells = list(stock_cells)
    label = mod.proxy_label(proxy)

    def put(column, value):
        cells[1 + header.index(column)] = value

    put("LABEL", label)
    put("Constant", f"FEAT_{label}")
    put("MINSPELLLVL", BLANK)
    put("MinLevel", str(mod.PROXY_MIN_LEVEL))
    put("MinLevelClass", str(proxy.cls.class_id))

    successor = stock_cells[1 + header.index("SUCCESSOR")]
    if successor.isdigit():
        heir = proxies_by_stock.get((proxy.cls.name, int(successor)))
        if heir is not None:
            put("SUCCESSOR", str(heir.row))
        elif int(successor) in gated:
            put("SUCCESSOR", BLANK)

    cells[0] = str(proxy.row)
    return cells


def proxy_nss_include(proxy_list, mod):
    """Render unpacked/castfeat_ids.nss - the {proxy -> real, class} map.

    Accessors rather than a lookup table: the resolver walks the whole map in
    BOTH directions every time (the pairing invariant), so it wants to iterate.

    THE MAP IS NOT ONE-TO-ONE, and the class column is what makes that safe.
    Every casting class gets its own proxy for the same feat, so Silent Spell
    has seven of them. The forward direction does not care - any proxy grants
    the one real feat. The REVERSE direction cares completely: without the class
    it would see a wizard holding Silent Spell and grant him the sorcerer,
    cleric, druid, bard, paladin and ranger proxies as well.
    """
    lines = [
        ...
        "",
        f"const int CASTFEAT_COUNT = {len(proxy_list)};",
        "",
        "// The inert row the player actually picks.",
        "int CastFeat_ProxyAt(int nIndex);",
        "// The real feat it stands in for. NOT unique: one per casting class.",
        "int CastFeat_RealAt(int nIndex);",
        "// The class the proxy belongs to (classes.2da row). The reverse",
        "// direction needs it - see CastFeat_Resolve.",
        "int CastFeat_ClassAt(int nIndex);",
        "",
        "int CastFeat_ProxyAt(int nIndex)",
        "{",
        "    switch (nIndex)",
    ...
    for i, proxy in enumerate(proxy_list):
        lines.append(f"        case {i}: return {proxy.row};"
                     f"  // {mod.proxy_label(proxy)}")
    lines += [
        "    }",
        "    return -1;",
        "}",
    ...
    for i, proxy in enumerate(proxy_list):
        lines.append(f"        case {i}: return {proxy.stock_id};"
                     f"  // {proxy.stock_label}")
    lines += [
        "    }",
        "    return -1;",
    ...
    for i, proxy in enumerate(proxy_list):
        lines.append(f"        case {i}: return {proxy.cls.class_id};"
                     f"  // {proxy.cls.name}")
    lines += ["    }", "    return -1;", "}", ""]
    return "\n".join(lines)


def write_class_tables(proxy_list, mod, apply_changes):
    """Append each proxy to its class's cls_feat_*.2da.

    The class bonus feat list is drawn STRICTLY from these tables, so a proxy
    that is only in feat.2da can never appear as a bonus feat - which is the
    half of the defect the player actually reported. A proxy whose stock feat is
    reachable only through ALLCLASSESCANUSE gets no entry here, matching how the
    stock feat reaches that class.

    Stock rows are kept as their ORIGINAL LINES, not re-rendered. These tables
    are ragged - stock appended to them across several patches and the columns
    do not line up - so reformatting would rewrite every row and bury the two
    lines that actually changed. Same rule as feat.2da's base rows.
    """
    changed = []
    by_class = {}
    ...
    for cls in mod.PROXY_CLASSES:
        ...
        index_i = 1 + header.index("FeatIndex")
        list_i = 1 + header.index("List")
        label_i = 1 + header.index("FeatLabel")
        granted_i = 1 + header.index("GrantedOnLevel")
        menu_i = 1 + header.index("OnMenu")
        ...
        for offset, proxy in enumerate(rows_for_class):
            ...
            cells[0] = str(len(stock_lines) + offset)
            cells[label_i] = mod.proxy_label(proxy)
            cells[index_i] = str(proxy.row)
            cells[list_i] = proxy.list_value
            cells[granted_i] = "-1"
            cells[menu_i] = rows_stock_menu(stock_rows, menu_i, proxy.stock_id)
            appended.append(render(cells, widths))

        text = newline.join(preamble + stock_lines + appended) + newline
...
def rows_stock_menu(stock_rows, menu_i, stock_id):
    """The stock entry's OnMenu, so a proxy sits in the same submenu."""
    cells = stock_rows.get(stock_id)
    if cells is not None and len(cells) > menu_i:
        return cells[menu_i]
    return "0"
...
def main():
    ...
    # The caster feat proxies append after the legendary block. Their indices
    # therefore move if a legendary feat is ever inserted rather than appended -
    # which is already forbidden for the same reason, and the gate asserts it.
    mod = load_proxies()
    proxy_first = FIRST_ROW + len(FEATS)
    # `kept` holds raw lines, except for the rows stock_overrides() rewrote,
    # which are already cell lists. The derivation wants cells either way.
    stock_cells = {i: (c if isinstance(c, list) else c.split())
                   for i, c in kept.items()}
    gated = mod.gated_feats(header, stock_cells)
    proxy_list = mod.proxies(proxy_first, header, stock_cells)
    proxies_by_stock = {(p.cls.name, p.stock_id): p for p in proxy_list}
    for proxy in proxy_list:
        owned_cells[proxy.row] = build_proxy_cells(
    ...
    for proxy in proxy_list:
        print(f"        {proxy.row}  FEAT_{mod.proxy_label(proxy)}  "
              f"-> {proxy.stock_label} ({proxy.stock_id}), "
              f"needs spell level {proxy.min_spell_level}")
    ...
    pinc = proxy_nss_include(proxy_list, mod)
    pinc_same = (PROXY_IDS_INC.exists()
                 and PROXY_IDS_INC.read_text(encoding="utf-8") == pinc)
    PROXY_IDS_INC.write_text(pinc, encoding="utf-8")
    print(f"[feat] {'unchanged' if pinc_same else 'wrote'} -> {PROXY_IDS_INC}")
    unchanged = unchanged and pinc_same and all(s for _, _, _, s in tables)
    if not unchanged:
        print()
