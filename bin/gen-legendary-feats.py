...
def proxy_nss_include(proxy_list, mod):
    """Render unpacked/castfeat_ids.nss - the {proxy -> real} map.

    Two arrays rather than a switch: the resolver walks the whole map in BOTH
    directions every time (the pairing invariant), so it wants to iterate, not
    look up.
    """
    lines = [
