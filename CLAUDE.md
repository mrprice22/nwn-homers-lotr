...
| `palette_map.json` | Find **where a blueprint lives in the in-game toolset palette** (`Top > Sub > … category`) — the answer to "which palette folder is this creature/item/placeable in?". Keyed `resref → {name, type, palette, in_palette}` across all 9 palettes. Blueprints that exist but were never filed into a custom palette category (script-created / orphaned, e.g. `slot_token`) are included with `in_palette: false` — they have no palette folder in the toolset. **Built on demand by `bin/gen-palette-map.py`** (or the roadmap editor's *Palette Finder → Refresh* button), **not** by the wiki build — so re-run that script after adding/moving blueprints; it may be stale or absent otherwise. |
