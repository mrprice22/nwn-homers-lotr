...
**Droppability is gated differently from the rest.** A placement that disagrees with its
blueprint about the `Dropable` flag gives loot that works exactly once per reboot — as
placed until that creature first dies, as the blueprint says forever after. Which side is
right is a judgement call (intended loot the blueprint forgot, vs. a hand-edited
placement), so the 11 cases that predate the check are grandfathered in
`tests/known_loot_divergence.json` and merely **reported**; anything else fails the build.
The decision table is [CLAUDE-loot-divergence-audit.md](CLAUDE-loot-divergence-audit.md),
generated by `bin/audit-loot-divergence.py` (`--baseline` rewrites the known-list). The
gate also names baseline entries that no longer diverge, so the file cannot rot silently.
...
- [CLAUDE-loot-divergence-audit.md](CLAUDE-loot-divergence-audit.md) — every place a creature's placed instance disagrees with its blueprint about whether an item is **droppable** (loot that works exactly once per reboot, then reverts on respawn). **Generated** by `bin/audit-loot-divergence.py`; the existing cases are grandfathered in `tests/known_loot_divergence.json` and reported rather than gated, while new ones fail the build
