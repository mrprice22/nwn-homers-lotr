...
toolset palette with `python3 bin/file-palette-orphans.py --apply` — it appends the
blueprint to its matching `*palcus.itp.json` category (learned from existing members;
unknowns go to a "Module Specific*" fallback), so DMs can find/place it in the toolset.
The **`check_palette_coverage` smoke gate aborts the repack** until every blueprint is
filed, so this isn't optional. Standalone (dry-run by default) — **not** a wiki refresh.
